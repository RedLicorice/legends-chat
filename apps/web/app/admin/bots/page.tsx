...
export default async function AdminBotsPage() {
  ...
  const [botList, topicList] = await Promise.all([
    db.select({ id: bots.id, name: bots.name, avatarUrl: bots.avatarUrl, description: bots.description, webhookUrl: bots.webhookUrl, isActive: bots.isActive, createdAt: bots.createdAt, role: bots.role, roleExpiresAt: bots.roleExpiresAt, roleFallback: bots.roleFallback }).from(bots).orderBy(bots.createdAt),
